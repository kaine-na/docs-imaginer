...
The API has two main subscription plans which determine your concurrency and request limits. Each request to the API consumes RPM (Requests Per Minute) and Worker slots based on the model and quality chosen.
...
When calling `POST /api/public/v1/generate`, you must supply a valid `model_id`. Below is the dictionary of supported models and their specific parameters.
